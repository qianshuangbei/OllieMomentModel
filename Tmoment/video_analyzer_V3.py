...
def process_frame(frame, seg_model, pose_model, max_persons=4, frame_idx=None, isdebug=False):
    ...
    if isdebug:      
        cv2.imwrite(f'Tmoment/debug/pose/frame_{frame_idx:04d}_.jpg', frame)
...
def main(target_fps=5, rotate=0, isdebug=False):
    ...
    parser.add_argument('--model', type=str, help='Path to checkpoint to resume training from')
    parser.add_argument('--video', type=str, default='demo/test1.mp4', help='Path to input video')
    ...
    classifier = PoseClassifierV1(LABEL_size).to(device)
    ...
    if os.path.exists('Tmoment/debug'):
        shutil.rmtree('Tmoment/debug')
    os.makedirs('Tmoment/debug/image', exist_ok=True)
    os.makedirs('Tmoment/debug/segment', exist_ok=True)
    os.makedirs('Tmoment/debug/pose', exist_ok=True)
    ...
    while cap.isOpened():
        ...
        with torch.no_grad():
            ...
            weights = torch.tensor([0.8, 1.2, 0.8, 0.8, 1.0]).to(device)
            weighted_outputs = probs * weights  # Element-wise multiplication
            predictions = torch.argmax(weighted_outputs, dim=1)
    ...
    output_dir = 'Tmoment/output'
    cut_video(args.video, cut_results['cut_points'], output_dir)
